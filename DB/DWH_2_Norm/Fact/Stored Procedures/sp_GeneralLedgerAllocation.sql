...
CREATE PROC Fact.sp_GeneralLedgerAllocation
AS
set XACT_ABORT on 
set NOCOUNT on
...
INTO #AllocationValues
FROM    Norm.n_GeneralLedgerAllocation ngla
INNER JOIN (SELECT *, _isFirst = LAG(0,1,1) OVER (PARTITION BY ngl.GeneralLedger_bkey ORDER BY ngl.SysValidFromDateTime DESC) FROM Norm.n_GeneralLedger ngl) t 
    ON ngla.GLAccount_From_bkey = t.GLAccount_bkey AND ngla.LegalEntity_From_bkey = t.LegalEntity_bkey AND t.Calender_ReportingPeriod_bkey = ngla.Calender_From_bkey
WHERE t._isFirst = 1 AND (t.FinanceSegment_bkey Like ('1#%') or t.FinanceSegment_bkey Like ('-1'))
...
IF 0 <> (SELECT ROUND(SUM(AmountLCY), 0) FROM #CounterAllocation ca)
	RAISERROR ('Error in sp_GeneralLedgerAllocation. The allocation and counter allocation do not sum up to zero' , 16, 1); 

SELECT  ngl.AllocationPrinciple_bkey
       ,ngl.GLAccount_bkey
