CREATE PROC SalesML.[test that no errors in the SSIS ETL Log]
AS
BEGIN
  ...
  WITH CTE AS
    ...
    WHERE   SysBatchLogExecutionLog_key = (SELECT MAX(pev.SysExecutionLog_key) FROM DWH_0_Admin.Logging.PackageExecutionV pev WHERE pev.PackageName = 'Sales_Master')
    UNION ALL
