CREATE TABLE [BamsNo_RawTyped].[r_OpocCustomer] (
    [OpocCustomer_key]             INT            IDENTITY (1, 1) NOT NULL,
    ...
    [OpocCustomer_bkey]            NVARCHAR (100) NOT NULL,
    ...
    CONSTRAINT [PK_BamsNo_RawTyped_r_OpocCustomer] PRIMARY KEY CLUSTERED ([OpocCustomer_bkey] ASC, [SysValidFromDateTime] DESC) WITH (DATA_COMPRESSION = PAGE)
);
...
GO
CREATE NONCLUSTERED INDEX [NCIDX_SysModifiedUTC_BamsNo_RawTyped_r_OpocCustomer]
    ON [BamsNo_RawTyped].[r_OpocCustomer]([SysModifiedUTC] ASC) WITH (DATA_COMPRESSION = PAGE);
