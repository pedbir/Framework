...
GO
ALTER DATABASE [$(DatabaseName)]
    ADD FILE (NAME = [Norm_Index_5D742D38], FILENAME = N'$(DefaultDataPath)$(DefaultFilePrefix)_Norm_Index_5D742D38.mdf') TO FILEGROUP [Norm_Index];
...
GO
ALTER DATABASE [$(DatabaseName)]
    ADD FILE (NAME = [Norm_Data_7D8BAD62], FILENAME = N'$(DefaultDataPath)$(DefaultFilePrefix)_Norm_Data_7D8BAD62.mdf') TO FILEGROUP [Norm_Data];
...
CREATE TABLE [Norm].[n_Opportunity] (
    ...
    [Employee_SalesResponsible_bkey] NVARCHAR (100)  NULL,
...
GO
PRINT N'Creating [Norm].[n_Opportunity].[IX_Opportunity_AccessBkey]...';


GO
CREATE NONCLUSTERED INDEX [IX_Opportunity_AccessBkey]
    ON [Norm].[n_Opportunity]([Access_bkey] ASC, [SysValidFromDateTime] DESC)
    ON [Norm_Data];
...
CREATE VIEW Norm.vn_Access AS
							...
							SELECT  t1.Access_key, t2.SysExecutionLog_key, t2.SysDatetimeInsertedUTC, t2.SysDatetimeUpdatedUTC, t2.SysDatetimeDeletedUTC, t2.SysDatetimeReprocessedUTC, t2.SysModifiedUTC, t2.SysIsInferred, t2.SysValidFromDateTime, t2.SysSrcGenerationDateTime, t2.Access_bkey, t2.Calenar_Installed_bkey, t2.AccessCategory_Lvl1_bkey, t2.AccessCategory_Lvl2_bkey, t2.AccessCategory_Lvl3_bkey, t2.AccessCategory_Lvl4_bkey, t2.AccessCategory_Lvl5_bkey, t1._isLast
							FROM temp t1 
...
CREATE VIEW Norm.vn_Opportunity AS
							...
							SELECT  t1.Opportunity_key, t2.SysExecutionLog_key, t2.SysDatetimeInsertedUTC, t2.SysDatetimeUpdatedUTC, t2.SysDatetimeDeletedUTC, t2.SysDatetimeReprocessedUTC, t2.SysModifiedUTC, t2.SysIsInferred, t2.SysValidFromDateTime, t2.SysSrcGenerationDateTime, t2.Opportunity_bkey, t2.Area_bkey, t2.Access_bkey, t2.AddressMaster_bkey, t2.Employee_SalesResponsible_bkey, t2.OpportunityName, t2.SugarEnum_BusinessType_bkey, t2.CustomerType, t2.AddressStreetAndNo, t2.AddressPostalCode, t2.AddressStreetName, t2.AddressStreetNo, t2.AddressCity, t2.AddressCountry, t2.Latitude, t2.Longitude, t2.Fastighetsbeteckningc, t2.AcquiredAccess, t1._isLast
							FROM temp t1 
...
WITH temp AS (SELECT	Subscription_key
						...
						, Calendar_ToSubscription_bkey = LEAD(Calendar_Subscription_bkey, 1, GETDATE()) OVER (PARTITION BY Subscription_bkey ORDER BY Calendar_Subscription_bkey)
			FROM		Norm.n_Subscription
			WHERE Subscription_bkey <> -1)
	, temp2 AS (SELECT	t.Subscription_key
						...
						, FromSubscription = CAST(DATEADD(DAY, -DAY(t.Calendar_FromSubscription_bkey) + 1, t.Calendar_FromSubscription_bkey) AS DATE)
						, ToSubscription = EOMONTH(t.Calendar_ToSubscription_bkey)
						, NoOfSubscriptions = 1
				FROM	temp t
				WHERE	t.IsClosed																			= 0
						AND DATEDIFF(DAY, t.Calendar_FromSubscription_bkey, t.Calendar_ToSubscription_bkey) > 0)
SELECT	t2.SysDatetimeDeletedUTC
			, t2.SysModifiedUTC
			, t2.SysValidFromDateTime
			, t2.SysSrcGenerationDateTime
			, Subscription_key = CAST(CAST(t2.Subscription_bkey AS NVARCHAR(100)) + '#' + CONVERT(NVARCHAR(8), nc.Calendar_bkey, 112) AS NVARCHAR(100))
			, t2.Subscription_bkey
			, SubscriptionProduct_key = ISNULL(nsp.SubscriptionProduct_key, -1)
			, ServiceProvider_key
			, va.Access_key
			, t2.Access_bkey
			, Opportunity_key = ISNULL(vo.Opportunity_key, -1)
			, Area_key = ISNULL(na.Area_key, -1)
			, Calendar_Purchase_bkey = CAST(CONVERT(NVARCHAR(8), t2.Calendar_Purchase_bkey, 112) AS INT)
			, Calendar_Subscription_bkey = CAST(CONVERT(NVARCHAR(8), nc.Calendar_bkey, 112) AS INT)
			, t2.ResoposibleSalesEntity
			, t2.NoOfSubscriptions
FROM	temp2 t2
INNER JOIN Norm.vn_Calendar nc ON nc.Calendar_bkey BETWEEN t2.FromSubscription AND t2.ToSubscription AND nc.Calendar_bkey = EOMONTH(nc.Calendar_bkey)
LEFT JOIN Norm.vn_SubscriptionProduct nsp ON nsp.SubscriptionProduct_bkey = t2.SubscriptionProduct_bkey AND nsp._isLast = 1
LEFT JOIN Norm.vn_ServiceProvider vsp ON vsp.ServiceProvider_bkey = t2.ServiceProvider_bkey AND vsp._isLast = 1
LEFT JOIN Norm.vn_Access va ON va.Access_bkey = t2.Access_bkey AND va._isLast = 1
OUTER APPLY (SELECT TOP 1 * FROM Norm.n_Opportunity vo WHERE vo.Access_bkey = va.Access_bkey ORDER BY vo.SysValidFromDateTime DESC ) vo
LEFT JOIN Norm.vn_Area na ON na.Area_bkey = vo.Area_bkey AND na._isLast = 1
GO
...
SELECT  vo.SysSrcGenerationDateTime
        ...
        , SalesOrder_key  = -1
        , Customer_key    = -1
		, Employee_SalesResponsible_key = ne.Employee_key
        , na2.Access_key
...
CROSS APPLY (SELECT TOP 1 * FROM Norm.n_Employee ne WHERE ne.Employee_bkey = vo.Employee_SalesResponsible_bkey ORDER BY ne.SysValidFromDateTime DESC) ne
...
CROSS APPLY (SELECT TOP 1 * FROM Norm.n_Employee ne WHERE ne.Employee_bkey = no.Employee_SalesResponsible_bkey ORDER BY ne.SysValidFromDateTime DESC) ne
...
OUTER APPLY (SELECT TOP 1 * FROM Norm.n_Opportunity no WHERE no.Access_bkey = na2.Access_bkey ORDER BY no.SysValidFromDateTime DESC) vo
...
CROSS APPLY (SELECT TOP 1 * FROM Norm.n_Employee ne WHERE ne.Employee_bkey = vo.Employee_SalesResponsible_bkey ORDER BY ne.SysValidFromDateTime) ne
